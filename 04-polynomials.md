...
복소수 $z \in \mathbb{C}$를 순서쌍 $(\operatorname{Re}z,\operatorname{Im}z)\in\mathbb{R}^2$와 동일시하면 $\mathbb{C}$를 $\mathbb{R}^2$와 동일시할 수 있다. $\mathbb{C}$는 $1$차원 복소 벡터공간이지만, $\mathbb{R}^2$와 동일시된 $\mathbb{C}$를 $2$차원 실수 벡터공간으로 생각할 수도 있다.
...
1. $w,z\in\mathbb{C}$라고 하자. 다음 등식과 부등식을 확인하라.
   ...
   (f) $|\operatorname{Re}z|\le |z|$ 그리고 $|\operatorname{Im}z|\le |z|$
